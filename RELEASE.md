...
```bash
make gh-release NEW_VERSION=v0.8.1 OLD_VERSION=v0.5.0 DESCRIPTION="A test release to see how it works"
```
...
> :warning: **NOTE:** when you need to test some changes in `.goreleaser.yml`, also apply the changes to `.goreleaser.draft.yml`. Then make sure your new `tag` ends with `-draft` (e.g.: `v0.8.1-draft`) to make a draft release to not notify our consumers when testing updates to the release process.
