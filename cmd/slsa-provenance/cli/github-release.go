...
func GitHubRelease(w io.Writer) *ffcli.Command {
	var (
		flagset = flag.NewFlagSet("slsa-provenance generate github-release", flag.ExitOnError)
...
(if set the artifacts will be downloaded from the release and the provenance will be added as an additional release asset.)`)
	...
	return &ffcli.Command{
		Name:       "github-release",
		ShortUsage: "slsa-provenance generate github-release",
		ShortHelp:  "Generates slsa provenance for assets in a GitHub release",
		FlagSet:    flagset,
		Exec: func(ctx context.Context, args []string) error {
			...
			if *tagName == "" {
				flagset.Usage()
				return RequiredFlagError("-tag_name")
			}
